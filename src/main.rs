...
pub async fn main() -> reqwest::Result<()> {
    ...
    // TODO only start when clicking text field
    ...
    let font = font_context
        .load_font(path, 40)
        .expect("couldn't load font"); // 2x larger text

    let texture_creator = canvas.texture_creator();

    let (render_width, render_height) = canvas.window().drawable_size();
    let (win_width, win_height) = canvas.window().size();
    if render_width / win_width != render_height / win_height {
        panic!("what the heck?");
    }
    let hidpi_scale = render_width / win_width;
    ...
    'running: loop {
        ...
        // TODO rendering logic for the UI should all be in Layout, and the Layout should have all the
        // relevant UI elements contained (and contact_list should be a struct as well)
        canvas.set_draw_color(Color::RGB(47, 49, 54));
        canvas.draw_rect(Rect::new(0, 0, contact_list_width, render_height)).expect("couldn't draw contact list");

        // TODO this should be a "Contact" UI element
        ...
        let button_color = if let Some(_) = layout.selected_contact { Color::RGB(74, 144, 226) } else { Color::RGB(64, 68, 75) };
        ...
        canvas
            .copy(
                &button_texture,
                None,
                Rect::new(text_x, text_y, button_query.width, button_query.height),
            )
            .unwrap();

        // Draw messages if contact is selected
        if let Some(contact) = &layout.selected_contact {
            ...
            let mut y_offset = chat_start_y + layout.chat_state.as_mut().unwrap().scroll_offset;

            for message in &layout.chat_state.as_ref().unwrap().messages {
                ...
                if y_offset + message_height > chat_start_y && y_offset < chat_area_bottom {
                    message.draw_bubble(
                        Point::new(chat_area_x as i32, y_offset),
                        &mut canvas,
                        &font,
                        chat_area_width - 40,
                        &texture_creator,
            ...
            let instruction_surface = font
                .render(instruction_text)
                .solid(Color::RGB(150, 150, 150))
                .unwrap();
            ...
            let center_x =
                chat_area_x as i32 + (chat_area_width as i32 - instruction_query.width as i32) / 2;
            ...
            canvas
                .copy(
                    ...
                    Rect::new(
                        center_x,
                        center_y,
                        instruction_query.width,
                        instruction_query.height,
                    ),
                )
                .unwrap();
        }

        // Draw text input area if chat is going
        if let Some(_) = &layout.chat_state {
            let input_area_height = 80;
            ...
            canvas
                .fill_rect(Rect::new(
                        ...
                        input_y,
                        chat_area_width,
                        input_area_height as u32,
                ))
                .expect("couldn't draw input area");

            // Draw input text
            let input_display = if layout.chat_state.as_mut().unwrap().input_text.is_empty() {
                "Type a message...".to_string()
            } else {
                layout.chat_state.as_mut().unwrap().input_text.clone()
            };

            let input_color = if layout.chat_state.as_mut().unwrap().input_text.is_empty() {
        ...
        canvas
            .fill_rect(Rect::new(
                status_button_x,
                status_button_y,
                status_button_width as u32,
                status_button_height as u32,
            ))
            .unwrap();
        ...
        let status_text_y =
            status_button_y + (status_button_height - status_query.height as i32) / 2;
        canvas
            .copy(
                &status_texture,
                None,
        ...
        for event in event_pump.poll_iter() {
            match event {
                ...
                Event::MouseButtonDown {
                    ...
                    let button_w =
                        (contact_list_width as i32 - (button_padding * 2)) / hidpi_scale as i32;
                    ...
                    if x >= button_x
                        ...
                        && y < button_y + button_h
                    {
                        if layout.selected_contact.is_some() {
                            layout.deselect_contact();
                        } else {
                            layout.select_contact(0); // just selecting server guy for now
                        }
                    }
                }
                Event::TextInput { text, .. } => {
                    if layout.chat_state.is_none() { continue; }
                    for ch in text.chars() {
                        layout.chat_state.as_mut().unwrap().handle_char_input(ch);
                    }
                }
                Event::KeyDown {
                    ...
                    if let Some(_) = layout.selected_contact { layout.send_message(); }
                }
                Event::KeyDown {
                    ...
                    if layout.chat_state.is_none() { continue; }
                    layout.chat_state.as_mut().unwrap().handle_backspace();
                }
                Event::MouseWheel { y, .. } => {
                    if layout.selected_contact.is_some() {
                        if y > 0 {
                            layout.chat_state.as_mut().unwrap().scroll_up();
                        } else if y < 0 {
                            layout.chat_state.as_mut().unwrap().scroll_down();
                        }
                    }
                }
                Event::Window {
